...
def read(file, readmarkers=False, readmarkerlabels=False, readmarkerslist=False, readloops=False, readpitch=False,
         normalized=False, forcestereo=False, log=True, readlistinfo=True, readunsupported=True):
    """
    ...
    * The data is returned as a numpy array with a
    ...
    unsupported = {}
    loops = []
    list_info_index = ["IARL", "IART", "ICMS", "ICMT", "ICOP", "ICRD", "IENG", "IGNR", "IKEY", "IMED", "INAM", "IPRD",
                       "ISBJ", "ISFT", "ISRC", "ISRF", "ITCH"]
    info = {}
    pitch = 0.0
    while (fid.tell() < fsize):
        ...
        if chunk_id == b'fmt ':
            ...
            for i in range(numsampleloops):
                ...
                loops.append(
                    {'cuepointid': cuepointid, 'datatype': datatype, 'start': start, 'end': end, 'fraction': fraction,
                     'playcount': playcount})
        else:
            if log:
    ...
    * To write multiple-channels, use a 2-D array of shape
    ...
    if loops or pitch:
        ...
        for i, loop in enumerate(loops):
            fid.write(struct.pack('<iiiiii', loop['cuepointid'], loop['datatype'], loop['start'], loop['end'],
                                  loop['fraction'], loop['playcount']))
    ...
    if infos:
        ...
        fid.write(struct.pack('<i', size))
        fid.write(
            b'INFO')  # https://web.archive.org/web/20141226210234/http://www.sonicspot.com/guide/wavefiles.html#list
        fid.write(info)
